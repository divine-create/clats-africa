    ...
    <div className='flex flex-col gap-3 w-full'>
      ...
      <div className='flex gap-1 bg-slate-200 dark:bg-slate-800 p-1 rounded-lg mt-3'>
        {tabs.map(t => (
          <button
            ...
            className={`flex-1 text-[10px] font-bold py-1.5 rounded-md transition-all ${activeTab === t.id ? "bg-white dark:bg-slate-700 shadow" : "text-slate-500"}`}
          >
            {t.label}
          </button>
        ))}
      ...
      <div className='grid grid-cols-5 gap-2 mt-1 max-h-[120px] overflow-y-auto p-1'>
        {activeTab === 'skin' && SKIN_COLORS.map(c => (
          <button key={c} onClick={() => updateConfig('skinColor', c)} className={w-8 h-8 rounded-full border-2 } style={{ backgroundColor: '#' + c }} />
        ))}

        {activeTab === 'hair' && HAIR_STYLES.map(h => (
          <button key={h} onClick={() => updateConfig('top', h)} className={	ext-[10px] truncate rounded-md p-1 border }>
            {h.replace(/([A-Z])/g, ' ').trim()}
          </button>
        ))}

        {activeTab === 'clothes' && CLOTHING.map(c => (
          <button key={c} onClick={() => updateConfig('clothing', c)} className={	ext-[10px] truncate rounded-md p-1 border }>
            {c.replace(/([A-Z])/g, ' ').trim()}
          </button>
        ))}

        {activeTab === 'face' && [...EYES, ...ACCESSORIES].map(f => (
          <button key={f} onClick={() => updateConfig(EYES.includes(f) ? 'eyes' : 'accessories', f)} className={	ext-[10px] truncate rounded-md p-1 border }>
            {f}
          </button>
        ))}
      </div>
